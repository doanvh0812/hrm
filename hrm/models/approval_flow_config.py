    ...
    def _check_duplicate_config_office(self):
        ...
        if self.system_id:
            ...
            for system in self.system_id:
                ...
                print("Hệ thống", system.name, "không được cấu hình trong bản ghi này",
                      any(elem in system_have_child_company(system.name) for elem in self.system_id.ids))
                    ...
                    for record in record_temp_configured:
                        ...
                        for sys in record[1]:
                            ...
                            if sys.id == system.id and not any(
                                    elem in system_have_child_company(sys.name) for elem in record[1].ids):
